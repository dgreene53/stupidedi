        ...
        module SyntaxNotes
          ...
          class P < Schema::SyntaxNote
            def required(zipper)
              if zipper.node.edi_present?
                xs = children(zipper)
                xs.any?{|x| x.node.edi_present? } ? xs : []
              else
                []
              end
            end
            ...
            def reason(zipper)
              present = indexes.select{|n| zipper.child(n - 1).node.edi_present? }
          ...
          class E < Schema::SyntaxNote
            ...
            def forbidden(zipper)
              if zipper.node.edi_present?
                xs = children(zipper)
                xs.count{|x| x.node.edi_present? } <= 1 ? [] : xs
              else
                []
              end
            end
          ...
          class C < Schema::SyntaxNote
            def required(zipper)
              if zipper.node.edi_present?
                if zipper.child(indexes.head - 1).node.edi_present?
